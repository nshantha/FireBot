...
class NavTest():
    def __init__(self):
        ...
        #rospy.Subscriber('fire_locations',Pose, self.update_locations)
        ...
        rospy.Subscriber('cube_list', Marker)
    ...
    global prev
    prev=Pose()
    def update_locations(self,min_point):
        global flag
        global prev
        #print("SUBSCRIBED TO FIRE_PONITS")
        if min_point not in self.fire_points:
            self.fire_points.append(min_point)
            print(len(self.fire_points))
            prev=min_point
        
        if len(self.fire_points) == 3:       
            if flag == 0:
                for i,ele in enumerate(self.fire_points):
                    self.locations_dict[i]=ele
            flag = 1
